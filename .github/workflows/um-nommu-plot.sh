...
plot \
   ...
   '${OUTPUT}/out/um-nommu-seccomp-lmbench-out.dat' usin (\$0+0.2):(\$1) w boxes fill patter 2 lt 1 lc rgb "royalblue" title "um(nommu(s))" ,\
   '${OUTPUT}/out/um-nommu-zpoline-lmbench-out.dat' usin (\$0+0.4):(\$1) w boxes fill patter 2 lt 1 lc rgb "blue" title "um(nommu(z))"
...
for f in `ls $OUTPUT/*.dat |grep -v iperf3`
do
 export $(basename $f .dat|sed "s/-/_/g")=`grep aver $f | grep -v time | awk '{print $2}'`
done
echo -e "||native|um|um-mmu(s)|um-nommu(s)|um-nommu(z)|\n|--|--|--|--|--|--|"
echo "|getpid | ${native} | ${um_mmu} | ${um_mmu_seccomp} | ${um_nommu_seccomp}| ${um_nommu_zpoline}|"
...
plot \
   ...
   '${OUTPUT}/out/um-nommu-seccomp-iperf3-out.dat' usin (\$0+0.2):(\$1/1000) w boxes fill patter 2 lt 1 lc rgb "royalblue" title "um(nommu(s))" ,\
   '${OUTPUT}/out/um-nommu-zpoline-iperf3-out.dat' usin (\$0+0.4):(\$1/1000) w boxes fill patter 2 lt 1 lc rgb "blue" title "um(nommu(z))"
...
echo -e "||native|um|um-mmu(s)|um-nommu(s)|um-nommu(z)|\n|--|--|--|--|--|--|"
echo "|iperf3(f) | ${native_f} | ${um_mmu_f} | ${um_mmu_seccomp_f} | ${um_nommu_seccomp_f}| ${um_nommu_zpoline_f}|"
echo "|iperf3(r) | ${native_r} | ${um_mmu_r} | ${um_mmu_seccomp_r} | ${um_nommu_seccomp_r}| ${um_nommu_zpoline_r}|"
